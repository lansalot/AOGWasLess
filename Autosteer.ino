...
float pwmDriveF = 0;
float calculatedPosition = 0, aogCompatiblePosition = 0, aogCompatiblePositionOld = 0;
float kalman = 0.0, oldGyro = 0, gyro = 0, bump = 0;
//---------------------------------------------------------------------------
...
float steerAngleOld = 0;      // from old steering sensor error
...
void steerSettingsInit()
{
  ...
  if (steerConfig.SingleInputWAS) // Single Input ADS
    ...
    highLowPerDeg = ((float)(steerSettings.highPWM - steerSettings.lowPWM)) / 5;
}

bool getWasLessRun() { return !steerConfig.CytronDriver; }
  ...
  if (currentTime - autsteerLastTime >= LOOP_TIME)
  {
    ...
    if (steerConfig.ShaftEncoder)
    {
      if (!steerSwitch)
      {
        if (pulseCount)
        {
          ...
          if ((steerAngleError >= 0 && (steerAngleOld < 0 || steerAngleOld + 5 < steerAngleError)) || (steerAngleError < 0 && (steerAngleOld >= 0 || steerAngleOld - 5 > steerAngleError)))
          {
            pulseCount = 0;
          }
          else if (((steerAngleError >= 0 && steerAngleError >= steerAngleOld) || (steerAngleError < 0 && steerAngleError <= steerAngleOld)) && abs_AngleError > 1)
          {
            if (gpsSpeed < 1)
              pulseCount++;
            else
              pulseCount += constrain(abs_AngleError, gpsSpeed, 5);
          ...
          steerAngleOld = steerAngleError;
          sensorReading = pulseCount;
        }
        else
        {
          if (sensorReading > 245)
          {
            sensorReading = pulseCount = 1;
          }
          sensorReading += (LOOP_TIME / (steerConfig.PulseCountMax / 12));
        }
      }
      else
      {
    ...
    */
    ...
    else
    {
      ...
      {
        gyro = bno08x.getGyroZ() * -1000;
        if (steerConfig.IsUseY_Axis)
        {
          bump = bno08x.getAccelX();
        }
        else
        {
        ...
        if (gyro > oldGyro + wasLessConfig.accelRate || gyro < oldGyro - wasLessConfig.accelRate)
        {
          if (gyro > oldGyro && gyro > 0)
            gyro = oldGyro + wasLessConfig.accelRate / wasLessConfig.user4;
          if (gyro < oldGyro && gyro < 0)
            gyro = oldGyro - wasLessConfig.accelRate / wasLessConfig.user4;
        }
        if (steerConfig.SingleInputWAS || (abs(gyro) < wasLessConfig.rate && abs(bump) < wasLessConfig.bumpLimit))
        {
          kalman = simpleKalmanFilter.updateEstimate(gyro);
          oldGyro = gyro;
        }
        if (gpsSpeed > MIN_WASLESS_SPEED)
        {
          calculatedPosition = kalman / gpsSpeed;
          aogCompatiblePosition = (calculatedPosition * 100);
        ...
        {
          aogCompatiblePosition = aogCompatiblePositionOld;
        }
      }
  ...
  if (len > 4)
  {
    ...
    if (autoSteerUdpData[0] == 0x80 && autoSteerUdpData[1] == 0x81 && autoSteerUdpData[2] == 0x7F) // Data
    {
      if (autoSteerUdpData[3] == 0xFE && Autosteer_running) // 254
      {
        ...
        // Steer Data 2 -------------------------------------------------
        if (steerConfig.PressureSensor || steerConfig.CurrentSensor || steerConfig.ShaftEncoder)
        {
          // Send fromAutosteer2
      ...
      else if (autoSteerUdpData[3] == 0xFB) // 251 FB - SteerConfig
      {
        ...
        bool lastSettingCytronDriver = steerConfig.CytronDriver;
        ...
        if (steerConfig.CytronDriver != lastSettingCytronDriver)
      ...
      {
        wasLessConfig.rate = (autoSteerUdpData[9] * 40);
        wasLessConfig.accelRate = autoSteerUdpData[10];
        wasLessConfig.bumpLimit = (autoSteerUdpData[11] / 100);
        wasLessConfig.user4 = autoSteerUdpData[12];
